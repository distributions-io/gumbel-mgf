...
function mgf( t, options ) {
	...
	if ( isNumber( t ) ) {
		return mgf1( t, opts.mu, opts.beta );
	}
	if ( isMatrixLike( t ) ) {
		if ( opts.copy !== false ) {
		...
		return mgf5( out, t, opts.mu, opts.beta );
	}
	if ( isTypedArrayLike( t ) ) {
		...
		return mgf6( out, t, opts.mu, opts.beta );
	}
	if ( isArrayLike( t ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return mgf4( t, opts.mu, opts.beta, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return mgf3( out, t, opts.mu, opts.beta, opts.accessor );
		}
		return mgf2( out, t, opts.mu, opts.beta );
	}
	return NaN;
} // end FUNCTION mgf()
